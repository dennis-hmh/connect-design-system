...
export const Timer: React.FC<TimerProps> = ({
  ...
  useEffect(() => {
    ...
    if (isFinished){
      ...
      if (onTimeUp && onTimeUp instanceof Function)
        onTimeUp();
    }
  }, [componentState]);
  ...
  useEffect(() => {
    ...
    //Handle pausing
    if (isPaused){
      if (!countdownPausedAt.current)
        countdownPausedAt.current = Date.now();
    }
    ...
    intervalRef.current = setInterval(() => {
      setRemainingTime((prevTime) => {
        ...
        let newTime = time - (Date.now() - countdownStartedAt.current - countdownTotalTimeSpentPaused.current);
  ...
  var nearestMultiple = Math.max(0, Math.round(remainingTime/updateInterval)*updateInterval);
